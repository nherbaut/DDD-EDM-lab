...
CLASSIFIER_URL = os.getenv("CLASSIFIER_URL", "http://localhost:8000/predict")
...
def compute_backoff_seconds(attempt: int) -> float:
    # Exponential backoff with a small jitter to avoid retry spikes.
...
def classify_image(image_bytes: bytes) -> dict[str, Any]:
    LOGGER.info("Running inference via classifier API at %s", CLASSIFIER_URL)
    ...
    LOGGER.info("Inference response received (status=%s)", response.status_code)
...
def publish_invalid_document(channel, original_document: dict, reason: str, details: str) -> None:
    invalid_document = {
...
def handle_request_document(channel, body: bytes) -> None:
    ...
    try:
        ...
        raise
    LOGGER.info(
        "Cloud image received for cloudId=%s (%s bytes)",
        cloud_id,
        len(image_response.content),
    )

    prediction = classify_image(image_response.content)
...
def main() -> None:
    ...
    while True:
        try:
            ...
            LOGGER.exception(
                "RabbitMQ connection/consume error: %s. Retrying in %.1fs",
                exc,
                wait_s,
            )
            time.sleep(wait_s)
