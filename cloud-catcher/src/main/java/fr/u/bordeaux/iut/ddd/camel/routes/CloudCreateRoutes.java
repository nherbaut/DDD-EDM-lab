...
public class CloudCreateRoutes extends EndpointRouteBuilder {
    ...
    public void configure() {
        from(direct("emit-user-created-event"))
                ...
                .to("spring-rabbitmq:cloud-classifier-exchange?routingKey=admin.user.new&exchangeType=direct");

        from(springRabbitmq("cloud-classifier-exchange")
                .queues("cloud.user.created.events")
                ...
                .process(exchange -> LOG.infof("Created cloud-catcher user %s", exchange.getProperty("cloudcatcher.userName", String.class)))
                .otherwise()
                .process(exchange -> LOG.debugf("cloud-catcher user already exists %s", exchange.getProperty("cloudcatcher.userName", String.class)))
                .end();
        ...
        from(platformHttp("/quota-request").httpMethodRestrict("POST"))
                .process(RoleCheckProcessor.checkRole("viewer"))
                .process(exchange -> {
                    ...
                    int packageQuota = request.path("packageQuota").asInt(10);
                    String userName = exchange.getProperty("cloudcatcher.userName", String.class);
                    Map<String, Object> event = new HashMap<>();
                    event.put("documentType", "QuotaRequestV1");
                    event.put("userName", userName);
                    event.put("packageQuota", packageQuota);
                ...
                .to("spring-rabbitmq:cloud-classifier-exchange?routingKey=accounting.quota.request.v1&exchangeType=direct")
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(202))
                .setHeader(Exchange.CONTENT_TYPE, constant("text/plain"))
                .setBody(constant("Quota request accepted\n"));
    ...
    private class BuildCloudClassificationRequestMessage implements Processor {
        ...
        public void process(Exchange exchange) throws Exception {
            ...
            classificationRequest.put("requestId", UUID.randomUUID().toString());
